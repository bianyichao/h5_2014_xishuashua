<?php if (!defined('THINK_PATH')) exit();?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>帖子详情</title>
    <meta name="description" content="Source code generated using layoutit.com">
    <meta name="author" content="LayoutIt!">

    <link href="/h5_2014_xishuashua/xishuashua/Public/admin/css/bootstrap.min.css" rel="stylesheet">
    <link href="/h5_2014_xishuashua/xishuashua/Public/admin/css/style.css" rel="stylesheet">
    <link href="/h5_2014_xishuashua/xishuashua/Public/admin/css/index.css" rel="stylesheet">
	<link rel="stylesheet" href="/h5_2014_xishuashua/xishuashua/Public/admin/css/hanziqiang.css" />
	<link rel="stylesheet" href="/h5_2014_xishuashua/xishuashua/Public/admin/css/xq_navbar.css"/>
	<link rel="stylesheet" href="/h5_2014_xishuashua/xishuashua/Public/admin/css/xq.css"/>
	<link rel="stylesheet" href="/h5_2014_xishuashua/xishuashua/Public/admin/css/tiezi.css" />
	
 	<script src="/h5_2014_xishuashua/xishuashua/Public/admin/js/jquery.min.js"></script>
    <script src="/h5_2014_xishuashua/xishuashua/Public/admin/js/bootstrap.min.js"></script>
    <script src="/h5_2014_xishuashua/xishuashua/Public/admin/js/scripts.js"></script>
	<script src="/h5_2014_xishuashua/xishuashua/Public/admin/js/jquery-2.1.4.min.js"></script>
	<script src="/h5_2014_xishuashua/xishuashua/Public/admin/js/text.js"></script>
	<script src="http://cdn.bootcss.com/jquery/1.11.1/jquery.min.js"></script>
	<script src="http://cdn.bootcss.com/bootstrap/3.3.0/js/bootstrap.min.js"></script>
  </head>
  <body>

    <div class="container-fluid" style="background-image:url('images/background.jpg')">

<!--header-->
	<div class="containter">
		<nav class="navbar navbar-default navbar-fixed-top" role="navigation">
			<div class="row">
				<div class="col-md-2 nobo">
					<a href="index.html"><img src="/h5_2014_xishuashua/xishuashua/Public/admin/images/logo.png" style="height:50px; width:73%;padding-left: 1.6em "></a>
				</div>
				<div class="col-md-7 nobo">
				<div class="xq_bag" id="bar1">
					<ul class="xq_navbar">
						<li class="xq_navli"><a href="index.html">首页</a></li>
						<li class="xq_navli"><a href="zsgs.html">装饰公司</a></li>
						<li class="xq_navli"><a href="jzzp.html">家装招标</a></li>
						<li class="xq_navli"><a href="zxrj.html">装修日记</a></li>
					</ul>
					
				</div>
				</div>
				<div class="col-md-3 nobo">
					<div class="navright1">
						<a href="dl.html" class="dl">登录</a>&nbsp;&nbsp;
						<a href="regist.html" class="regist">注册</a>
					</div>
				</div>
			</div>	
		</nav>
	</div>
	<div class="container">
	<div class="row">
      <div class="col-md-1"></div>
        <div class="col-md-10">
		<div class="tabbable tabbable2"> <!-- Only required for left/right tabs -->
			<div class="tab-content">
				<div class="tab-pane active" id="tab1">	
					<!--content 1-->
					<div class="tiezi">
						<div class="tz_head">
							<h3 class="tz_title">【po进度】这是我家的装修进度，和大家分享</h3>
								<button onclick="msgbox(1)" class="reply">
									<span class="glyphicon glyphicon-comment" title="回复"></span>
									<em class="glyphicon-eyes">195</em>
								</button>
								<button class="reply" onclick="aa()">
									<span class="glyphicon glyphicon-star-empty" title="收藏"></span>
									<em class="glyphicon-eyes">95</em>
								</button>
								<button class="reply">
									<span class="glyphicon glyphicon-eye-open" title="浏览数"></span>
									<em class="glyphicon-eyes">95</em>
								</button>	
						</div>
						<div class="tz_text">
							<p class="tz_text_name_xq">&nbsp;帖主：<a href="grzx.html" class="vistor">李小姐</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;发帖时间：2011-11-11</p>
							<p class="tz_text_neirong">
							今天是装修第一天，师傅们看起来非常专业啊！我很有信心！对我的新家充满了期待！^_^
							</p>
							<img src="/h5_2014_xishuashua/xishuashua/Public/admin/images/roll03.jpg" class="tz_img">
							<img src="/h5_2014_xishuashua/xishuashua/Public/admin/images/roll04.jpg" class="tz_img">
							<img src="/h5_2014_xishuashua/xishuashua/Public/admin/images/roll02.jpg" class="tz_img">
						</div>
					</div>
				
					<div class="tiezi_1">
						<div class="tz_head">
							<p class="tz_text_huifu">
            					<a href="grzx.html" class="vistor">&nbsp;王家小妹:</a>	
            					<span class="reply_content">雾草雾草，这个师傅我曾经见过！上次还来我家帮我解决了好多建材的问题，很认真负责的师傅。</span>
							</p>
							<p>
							<span class="reply_time">回复时间：2016-11-30</span>
							<button onclick="msgbox(1)" class="reply_vistor">
									<span class="glyphicon glyphicon-comment" title="回复"></span>
							</button>
							</p>
							
						</div>
					</div>
					<div class="tiezi_1">
						<div class="tz_head">
							<p class="tz_text_huifu">
            					<a href="grzx.html" class="vistor">&nbsp;蓝瘦香菇:</a>	
            					<span class="reply_content">雾草雾草，这个师傅我曾经见过！上次还来我家帮我解决了好多建材的问题，很认真负责的师傅。</span>
							</p>
							<p>
							<span class="reply_time">回复时间：2016-11-30</span>
							<button onclick="msgbox(1)" class="reply_vistor">
									<span class="glyphicon glyphicon-comment" title="回复"></span>
							</button>
							</p>
							
						</div>
					</div>
					<div class="tiezi_1">
						<div class="tz_head">
							<p class="tz_text_huifu">
            					<a href="grzx.html" class="vistor">&nbsp;王大锤:</a>	
            					<span class="reply_content">雾草雾草，这个师傅我曾经见过！上次还来我家帮我解决了好多建材的问题，很认真负责的师傅。</span>
							</p>
							<p>
							<span class="reply_time">回复时间：2016-11-30</span>
							<button onclick="msgbox(1)" class="reply_vistor">
									<span class="glyphicon glyphicon-comment" title="回复"></span>
							</button>
							</p>
							
						</div>
					</div>
					<div class="tiezi_1">
						<div class="tz_head">
							<p class="tz_text_huifu">
            					<a href="grzx.html" class="vistor">&nbsp;麦兜:</a>	
            					<span class="reply_content">雾草雾草，这个师傅我曾经见过！上次还来我家帮我解决了好多建材的问题，很认真负责的师傅。</span>
							</p>
							<p>
							<span class="reply_time">回复时间：2016-11-30</span>
							<button onclick="msgbox(1)" class="reply_vistor">
									<span class="glyphicon glyphicon-comment" title="回复"></span>
							</button>
							</p>
							
						</div>
					</div>
					<div class="tiezi_1">
						<div class="tz_head">
							<p class="tz_text_huifu">
            					<a href="grzx.html" class="vistor">&nbsp;猪猪侠:</a>	
            					<span class="reply_content">雾草雾草，这个师傅我曾经见过！上次还来我家帮我解决了好多建材的问题，很认真负责的师傅。</span>
							</p>
							<p>
							<span class="reply_time">回复时间：2016-11-30</span>
							<button onclick="msgbox(1)" class="reply_vistor">
									<span class="glyphicon glyphicon-comment" title="回复"></span>
							</button>
							</p>
							
						</div>
					</div>
					<script type="text/javascript">
								function msgbox(n){
									document.getElementById('inputbox').style.display=n?'block':'none'; 
									document.getElementById("text-reply").focus();    /* 点击按钮打开/关闭 对话框 */
								}
							</script>
							<div class="tiezi_1 box" id="inputbox">
							<div class="tiezi_reply">
								<p>评论：</p>
								<form>
									<textarea class="text-reply" id="text-reply"></textarea>	
								</form>
								<button type="button" class="btn1" onclick="msgbox(0)">回复帖子</button>
							</div>
							</div>
				</div>
			</div>
		
		<div class="zx1">
            <nav>
              <ul class="pagination pagination-lg gs7">
                <li><a href="#">首页</a></li>
                <li class="gs8"><a href="#" >1</a></li>
                <li><a href="#">2</a></li>
                <li><a href="#">3</a></li>
                <li><a href="#">4</a></li>
                <li><a href="#">5</a></li>
                <li><a href="#">尾页</a></li>
              </ul>
            </nav>
            </div>
            </div>
            </div>
            <div class="col-md-1"></div>
            </div>
            </div>
<!--content-->

<!--footer-->
	<div class="row footer">
		<div class="col-md-10">
		<ul>
		    <li class="titli">相关链接</li>
			<li><a href="">河北省建材局</a></li>
			<li><a href="">河北省土木局</a></li>
			<li><a href="">东易日盛家装装饰有限公司</a></li>
			<li><a href="">乐豪家装装饰有限公司</a></li>
			<li><a href="">简家家装装饰有限公司</a></li>
		</ul>
		<ul>
		    <li class="titli">网站统计</li>
			<li>总访问量：1200045 人次</li>
			<li>今日访问：20178 人次</li>
			<li>注册人数：3759 人次</li>
			<li>注册公司数：170 所</li>
			<li>新增用户：235 人次</li>
		</ul>
		<ul>
		    <li class="titli">联系我们</li>
			<li>联系电话：xxx-xxx-xxxx</li>
			<li>邮编：xxxxxx</li>
			<li>电子邮箱：xxxx@xx.com</li>
			<li>公司地址：石家庄市 裕华区 裕华路 河北师大新校区内</li>
		</ul>
		</div>
		<div class="col-md-2">
		<p>扫码下载喜刷刷APP</p>
		<img src="/h5_2014_xishuashua/xishuashua/Public/admin/images/ewm.gif" alt="erweima"/>
		</div>
	</div>
	<div class="row cori">
		<div class="col-md-12">
		    <p>喜刷刷家装在线交流平台版权所有<br/>
		    Copyright © 2008－2016  All Rights Reserved .     网站备案：冀ICP备xxxxxxxx</p>
		</div>
	</div>
</div>

   
  </body>
  <script src="/h5_2014_xishuashua/xishuashua/Public/admin/js/xq_navbar.js"></script>
  <script>
    $(function(){
      $("#bar1").xq_navbar({"type":"underline","liwidth":"auto","bgcolor":"rgb(240,242,220)","hcolor":"rgb(1,175,99)"});
    });
  </script>
</html>